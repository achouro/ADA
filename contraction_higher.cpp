...
{
struct shortcut_{int from; int to; length dist; 
		shortcut_(int f, int t, length d):from(f), to(t), dist(d){} };
...
vector<shortcut_> shortcuts;

graph &g;
importance_queue imp_que;
vector<length> dist;

contraction_hierarchies(graph &g) : 
  g(g), n(g.adj[0].size()), dist(vector<length>(n,INF)),
...
for(auto in_edge : incoming_edges){
  ...
  for(auto out_edge: outgoing_edges){ 
    ...
    if(shortcut_needed){
      ...
      if(adding_shortcuts){
/*      shortcuts.emplace_back(shortcut_(in_edge.second, out_edge.second, in_edge.first + out_edge.first));*/

	shortcuts.push_back(shortcut_(in_edge.second, out_edge.second, in_edge.first + out_edge.first)); }}
