/* A* algorithm for Road Networks*/
/*Bidrectional Djikstra with Potentials*/
...
length euclidian(int s, int t){
  return sqrt(pow((coord[s].first-coord[t].first),2)+ 
	      pow((coord[s].second-coord[t].second),2) );}

length potential(int u){
...
void relax(que &queue, length d, int u, int v, int side){
  ...
  if(dist[side][u]> dist[side][v] +d +p){
    dist[side][u]=  dist[side][v] +d +p;
...
length compute_path(){
  ...
  return result==INF ? -1: (result + potential(s_)- potential(t_));} //return no path -1 or path length minus potential differential}


length bidirectional_astar(int s, int t){
  clear_containers();
  s_=s, t_=t;
...
while(q--){
  ...
  cout<<g.bidirectional_astar(--s,--t)<<endl;} }  //Bi-Djik ShPA 
