...
public:
    ...
    void add_outgoing_neighbour(const Vertex &to){ out.emplace_back(to);}

    void add_incoming_neighbour(const Vertex &from){ in.emplace_back(from);}

    int get_number_of_outgoing_neighbours(){ return out.size();}

    const int id() const {return this->index;}

    int get_number_of_incoming_neighbours(){ return in.size();}

    vector<Vertex> get_outgoing_neighbours(){ vector<Vertex> results{out};
					      return results;}

    vector<Vertex> get_incoming_neighbours(){ vector<Vertex> results{in};
                                              return results;}

    bool in_equal_out(){ return out.size()==in.size();}

    bool operator==(const Vertex &v) const{ return (this->index==v.id());}
};

namespace std{
    template <>
    struct hash<Vertex>{
	size_t operator()(const Vertex &v) const{ return((hash<int>()(v.id())));} }; }
...
public:
    Graph(const int &n_vertices, const int &n_edges) : n_vertices(n_vertices), n_edges(n_edges){
	...
	to.add_outgoing_neighbour(from); }
	...
	for(Vertex v: vertices){
	    ...
	    if(v.get_number_of_outgoing_neighbours()==0 &&
	       v.get_number_of_incoming_neighbours()==0) {return false;}

	    if(!v.in_equal_out()){ return false;} }
	return true; }
    ...
    void print_incoming_edges(Vertex v){
        vector<Vertex> in= v.get_incoming_neighbours();
        cout<<v.id()<< " outgoing: ";
        for(Vertex vn: in){
            cout<< " "<< vn.id()<< " "; }
        cout<<endl; }
	...
	if(eulerian_cycle_exists()){
	    ...
	    auto max_it= std::max_element(vertices.begin(),vertices.end(),
					[](auto &left, auto &right){
                                return left.get_number_of_outgoing_neighbours()>
right.get_number_of_outgoing_neighbours() ; } );
		...
		while(!cycle_found){
		    ...
		    while(neighbours[prev.id()].size()>1 &&
			  neighbours[cand->id()].size()==0){ ++cand;}
		    ...
		    if(prev.id()==start.id()){ cycle_found=true; } }

		
		bool at_least_one_vertex_unexplored=false;
		auto itr=current_cycle.begin();

		while(itr!= current_cycle.end() && !at_least_one_vertex_unexplored){
